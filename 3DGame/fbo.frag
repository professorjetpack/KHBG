...
void main(){
	...
	if(dead){
		...
		vec3 hdrColor = health >= 45 ? texture(hdrBuffer, texCoords).rgb : color / 25.0;
		...
		if(health < 60 && health > 25){
			FragColor *= vec4(1.0 + ((70.0 - health) / 100.0), 1.0 , 1.0, 1.0);
		}else if(health <= 25){
			FragColor *= vec4(1.0 + ((100.0 - health) / 100.0), 1.0 , 1.0, 1.0);
		}
